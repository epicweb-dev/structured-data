...
await testStep(
	'Order object should use OrderStatus enum correctly',
	async () => {
		enum OrderStatus {
			Pending = 'pending',
			Processing = 'processing',
...
await testStep('getStatusMessage should return correct messages', async () => {
	...
	).toBe('Your order is pending confirmation')
	expect(
	...
	).toBe('Your order is being processed')
	expect(
	...
	).toBe('Your order has been shipped!')
	expect(
